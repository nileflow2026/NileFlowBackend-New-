...
export const ProductsProvider = ({ children }) => {
    ...
    const updateProductRating = (productId, newRating) => {
        setProducts((prevProducts) => {
            return prevProducts.map((product) => product.id === productId
                ? { ...product, rating: newRating, totalRating: product.ratings.total + 1 }
                : product
            );
        });
    };
    ...
    return (
        <ProductsContext.Provider value={{ products, updateProductRating }}>
            {children}
